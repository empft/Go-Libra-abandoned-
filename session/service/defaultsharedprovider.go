...
func NewDefSharedProvider(namespace string, store zhStore) *DefSharedProvider {
...
func NewDefSharedProviderWithSeparateStore(namespace string, zStore kv.ZStore, hStore kv.HStore) *DefSharedProvider {
	return &DefSharedProvider{
		session: zStore,
		item: hStore,
		ns: namespace,
...
func (sp *DefSharedProvider) fetchAllSessions(ctx context.Context, id int) ([]session.Shared, error) {
	key := sp.idToTokenKey(id)
	...
	session := make([]session.Shared, len(res))
	for i, v := range res {
		session[i].Id = id
...
func (sp *DefSharedProvider) Init(ctx context.Context, id int) (*session.Shared, error) {
...
func (sp *DefSharedProvider) Read(ctx context.Context, ssid string) (*session.Shared, error) {
	sharedSession, err := session.NewShared(ssid)
	if err != nil {
		return nil, err
	}
...
func (sp *DefSharedProvider) FetchAll(ctx context.Context, ssid string) ([]session.Shared, error) {
