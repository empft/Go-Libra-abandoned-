...
func NewDefUniqueProvider(namespace string, store kv.ExpiringHStore) *DefUniqueProvider {
...
func (sp *DefUniqueProvider) Init(ctx context.Context, expiration time.Duration) (*session.Unique, error) {
	...
	return &session.Unique{Token: token}, err
}

func (sp *DefUniqueProvider) Read(ctx context.Context, ssid string, expiration time.Duration) (*session.Unique, error) {
	sess := session.NewUnique(ssid)
